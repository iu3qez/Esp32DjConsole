...
static const thetis_cmd_t s_cmd_db[] = {
    ...
    { 107, "VFO B Up 100kHz",    CAT_VFO,   CMD_CAT_BUTTON, "ZZBU", NULL,    0, 0, 0 },
    { 108, "VFO B Down 100kHz",  CAT_VFO,   CMD_CAT_BUTTON, "ZZBD", NULL,    0, 0, 0 },
    ...
    { 501, "Master Volume",      CAT_AUDIO, CMD_CAT_SET,    "ZZAG", NULL,    3, 0, 100 },
    ...
    { 507, "RX1 AGC Top",        CAT_AUDIO, CMD_CAT_SET,    "ZZAR", NULL,    3, 0, 120 },
    ...
    { 803, "RX1 AGC On/Off",     CAT_AGC,   CMD_CAT_TOGGLE, "ZZGC", NULL,   1, 0, 1 },
...
// VFO freq tracking for FREQ exec type
static long s_vfo_a_freq = 14074000;
static long s_vfo_b_freq = 7074000;
...
static void execute_command(const thetis_cmd_t *cmd, dj_control_type_t ctrl_type,
                            uint8_t old_val, uint8_t new_val, int32_t param)
{
    ...
    case CMD_CAT_FREQ: {
        ...
        if (ctrl_type == DJ_CTRL_ENCODER) {
            ...
            if (new_val == 0) return;
            delta = 1;  // Single step on button press
        } else {
            // Dial: treat value as direction indicator
            delta = (new_val > old_val) ? 1 : -1;
        }
        int step_hz = (param != 0) ? param : 10;  // Default 10 Hz step
        long *freq = NULL;
        if (strcmp(cmd->cat_cmd, "ZZFA") == 0) freq = &s_vfo_a_freq;
        else if (strcmp(cmd->cat_cmd, "ZZFB") == 0) freq = &s_vfo_b_freq;
        if (!freq) break;
        *freq += (long)delta * step_hz;
        if (*freq < 100000) *freq = 100000;       // 100 kHz floor
        if (*freq > 54000000) *freq = 54000000;    // 54 MHz ceiling
        snprintf(buf, sizeof(buf), "%s%011ld;", cmd->cat_cmd, *freq);
        cat_client_send(buf);
        ESP_LOGD(TAG, "FREQ [%s] delta=%d step=%d -> %ld Hz", cmd->name, delta, step_hz, *freq);
        break;
    }
...
void mapping_engine_reset_defaults(void)
{
    ...
    // -- Deck A --
    add_default("Jog_A",    100, 10);      // VFO A Tune, 10 Hz step
    add_default("Pitch_A",  100, 100);     // VFO A Tune, 100 Hz step
    ...
    add_default("CUE_A",    400, 0);       // MOX toggle (momentary not in new model)
    ...
    // FWD/RWD -> frequency step buttons
    add_default("FWD_A",    100, 1000);    // VFO A +1kHz
    add_default("RWD_A",    100, -1000);   // VFO A -1kHz
    add_default("FWD_B",    101, 1000);    // VFO B +1kHz
    add_default("RWD_B",    101, -1000);   // VFO B -1kHz
    ...
    uint8_t new_value)
{
    ...
    execute_command(cmd, control_type, old_value, new_value, m->param);
}
